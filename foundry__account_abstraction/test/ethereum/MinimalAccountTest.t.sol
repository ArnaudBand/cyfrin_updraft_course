...
contract MinimalAccountTest is Test {
  ...
  ERC20Mock usdc;
  SendPackedUserOp sendPackerUserOp;
  ...
  function setUp() public {
    ...
    usdc = new ERC20Mock();
    sendPackerUserOp = new SendPackedUserOp();
  }
  ...
  function testRecoverSignedOp() public {
    bytes memory executeCallData = abi.encodeWithSelector(MinimalAccount.execute.selector, address(usdc), 0, abi.encodeWithSelector(ERC20Mock.mint.selector, address(minimalAccount), INITIAL_BALANCE));
    PackedUserOperation memory packedUserOp = sendPackerUserOp.generateSignedPackedUserOp(executeCallData, helperConfig.getConfig());
