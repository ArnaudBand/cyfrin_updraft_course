...
contract MerkleAirdrop is EIP712 {
    ...
    function claim(address account, uint256 amount, bytes32[] calldata merkleProof, uint8 v, bytes32 r, bytes32 s) external {
        ...
        if (!_isValidSignture(account, getMessage(account, amount), v, r, s)) revert MerkleAirdrop__InvalidSignature(); // If the signature is invalid, revert
    ...
    function _isValidSignture(address account, bytes32 digest, uint8 v, bytes32 r, bytes32 s) internal pure returns (bool) {
